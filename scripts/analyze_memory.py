...
def parse_snapshot_index(csv_dir: str) -> List[SnapshotMeta]:
    """读取 snapshot_index.csv，返回所有成功采集的快照元数据。"""
...
def build_pagemap_index(pm_rows: List[PagemapRow]) -> Dict[Tuple[str, str], PagemapRow]:
    """
    建立 (sample_id, vma_id) → PagemapRow 的索引。
    vma_id 在 vma CSV 和 pagemap CSV 中一致（都是 vma_000001 格式）。
    """
    idx = {}
    for p in pm_rows:
...
def match_and_compare(
    ...
    for key, group in vma_index.items():
        ...
        present_count = 0  # 有多少个快照中 present_ratio > threshold

        for sid in sorted(sample_ids):
            if sid not in per_sample:
                # 这个快照中没有匹配到这个 VMA（可能在 fuzzy 模式下不存在）
                continue

            v = per_sample[sid][0]  # 同一 key 在同一快照中一般只有一个
...
示例:
  ...
  %(prog)s -i memcap_out/ --sample sample_001 sample_002  # 只对比指定快照
    ...
    parser.add_argument('--mode', choices=['auto', 'exact', 'fuzzy'], default='auto',
                        help='VMA 匹配模式 (默认: auto)')
    parser.add_argument('--threshold', type=float, default=0.8,
                        help='present_ratio 阈值，超过此值视为"物理驻留" (默认: 0.8)')
    parser.add_argument('--pid', type=int, default=None,
                        help='只分析指定 PID 的快照')
    parser.add_argument('--sample', nargs='*', default=None,
                        help='只对比指定的 sample_id (可多个)')
    ...
    # 1. 加载数据
    print(f"[加载] snapshot_index.csv ...", file=sys.stderr)
    ...
    if len(snapshots) < args.min_snapshots:
        print(f"[错误] 快照数量不足: {len(snapshots)} (需要 ≥ {args.min_snapshots})", file=sys.stderr)
        print(f"  提示: 多采集几次后再做对比分析", file=sys.stderr)
        sys.exit(1)

    print(f"[加载] {len(snapshots)} 个快照", file=sys.stderr)
    for s in snapshots:
        print(f"  {s.sample_id}  PID={s.pid}  {s.process_name}  {s.foreground_state}",
              file=sys.stderr)

    print(f"[加载] vma_memory_snapshot.csv ...", file=sys.stderr)
    vma_rows = parse_vma_csv(csv_dir)

    print(f"[加载] pagemap_snapshot.csv ...", file=sys.stderr)
    pm_rows = parse_pagemap_csv(csv_dir)
    pm_index = build_pagemap_index(pm_rows)

    # 2. 确定匹配模式
    mode = args.mode if args.mode != 'auto' else detect_mode(snapshots)
    print(f"[模式] {mode}", file=sys.stderr)

    # 3. 对比分析
    print(f"[分析] 跨快照匹配 VMA ...", file=sys.stderr)
    results = match_and_compare(snapshots, vma_rows, pm_index, mode, args.threshold)

    # 4. 输出报告
    print_report(results, args.threshold, len(snapshots), mode)
