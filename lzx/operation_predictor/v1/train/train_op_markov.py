#!/usr/bin/env python3
"""Train and evaluate the per-application operation Markov baseline."""
...
from v1.models.op_markov import OpMarkovModel
from src.utils.io_utils import load_json, load_pickle, save_csv


def evaluate(model: OpMarkovModel, test_samples: list[dict], top_k: list[int]) -> list[dict]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train in-app operation Markov baseline.")
    ...
    parser.add_argument("--output", default="outputs/results/v1/op_markov_results.csv")
...
def main() -> None:
    ...
    model = OpMarkovModel(order=args.order, op_vocab=op_vocab)
    ...
    print(f"apps in train: {len(model.global_counts)}")
