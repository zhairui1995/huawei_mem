#!/usr/bin/env python3
"""Train and evaluate the application-level Markov baseline."""
...
from src.models.app_markov import AppMarkovModel
from src.utils.io_utils import load_pickle, save_csv


def evaluate(model: AppMarkovModel, test_samples: list[dict], top_k: list[int], horizons: list[int]) -> list[dict]:
    rows: list[dict] = []
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train application Markov baseline.")
    ...
    parser.add_argument("--output", default="outputs/results/app_markov_results.csv")
...
def main() -> None:
    ...
    model = AppMarkovModel(order=args.order)
    ...
    fieldnames = ["model", "order", "horizon", "k", "hit_at_k", "recall_at_k", "precision_at_k", "mrr", "num_samples"]
