...
static void accumulate_summary(const LogicalRegionList *regions, long page_size, SegmentSummary sums[SEG_COUNT])
{
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        sums[s].rss_kb += (long)(r->pages.present_pages * (unsigned long long)page_size / 1024ULL);
...
static void print_section_summary(FILE *out, const LogicalRegionList *regions, long page_size)
{
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        row->sums.rss_kb += (long)(r->pages.present_pages * (unsigned long long)page_size / 1024ULL);
                                  ...
                                  const LogicalRegionList *regions, long page_size)
{
    ...
    fprintf(out, "## 总览矩阵\n\n");
    fprintf(out, "> 列为总进程与一级 segment；行为 `maps`、`smaps`、`pagemap` 指标。`路径/来源` 显示 file/anon/special 来源计数；分段列中的 VMA 数量按逻辑区间统计。\n\n");
                                      ...
                                      const VmaList *list, const LogicalRegionList *regions)
{
    ...
    print_target_segments(full, regions, page_size);
    print_vma_details(full, regions, page_size);
    print_vma_page_bitmaps(full, list, page_size);
    print_logical_pfn_sequences(full, regions, page_size);
    print_field_explanations(full);
    fclose(full);
    full = NULL;
    ...
    fprintf(w.index, "- [完整原始报告，用于人工追溯](full_report.md)\n");

    if (!print_split_target_segments(&w, regions, page_size) ||
        !print_split_vma_details(&w, regions, page_size) ||
        !print_split_vma_page_bitmaps(&w, list, page_size) ||
        !print_split_logical_pfn_sequences(&w, regions, page_size)) {
        split_writer_close_part(&w);
        fclose(w.index);
        return false;
    }
                                  ...
                                  char *detail_link, size_t detail_link_len)
{
    ...
    if (!write_split_detail_report(detail_path, detail_link_dir, pid, comm, exe_path,
                                   pagemap_status, page_size, &list, &regions)) {
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < pid_count; i++) {
        ...
        if (analyze_one_pid_split(pids[i], out, details_dir, detail_link, sizeof(detail_link))) {
            ok_count++;
        }
    }
