...
#define DETAIL_PART_LIMIT (200 * 1024)
#define DETAIL_ROTATE_LIMIT (180 * 1024)
#define MAX_REGION_CUTS 4096
#define SECTION_NAME_LEN 128
#define SECTION_LABEL_LEN 256
...
typedef struct {
    ...
    char path[PATH_MAX];
    SegmentKind segment;
    char section_label[SECTION_LABEL_LEN];
    char section_object[SECTION_NAME_LEN];
...
typedef struct {
    ...
    size_t file_sources;
    size_t anon_sources;
    size_t special_sources;
    size_t inode_sources;
    long size_kb;
    long rss_kb;
    long pss_kb;
    long swap_kb;
...
typedef struct {
    char label[SECTION_LABEL_LEN];
    SegmentKind segment;
    SegmentSummary sums;
} SectionSummaryRow;

typedef struct {
    SectionSummaryRow *items;
    size_t count;
    size_t cap;
} SectionSummaryList;
...
typedef struct {
    bool target_segments;
    bool vma_details;
    bool vma_page_bitmaps;
    bool logical_pfn_sequences;
    bool fields;
    bool full_report;
} ReportOptions;

static const ReportOptions DEFAULT_REPORT_OPTIONS = {0};
static const ReportOptions FULL_REPORT_OPTIONS = {
    .target_segments = true,
    .vma_details = true,
    .vma_page_bitmaps = true,
    .logical_pfn_sequences = true,
    .fields = true,
    .full_report = true,
};
...
static void usage(const char *argv0)
{
    fprintf(stderr,
            ...
            "单次分析 app: sudo %s --app firefox -o report.md\n"
            "也兼容 app 名: sudo %s firefox -o report.md\n"
            ...
            "\n"
            "Markdown 输出开关（默认只写总览/汇总表和 page_data.tsv）:\n"
            "  --md-preset compact|standard|full\n"
            "  --with-target-segments   写出重点分段字段\n"
            "  --with-vma-details       写出 VMA 明细表\n"
            "  --with-page-bitmaps      写出原始 VMA 页状态 Markdown\n"
            "  --with-pfn-markdown      写出逻辑区间 PFN Markdown\n"
            "  --with-fields            写出字段说明\n"
...
static bool set_report_preset(ReportOptions *opts, const char *preset)
{
    if (strcmp(preset, "compact") == 0) {
        *opts = DEFAULT_REPORT_OPTIONS;
        return true;
    }
    if (strcmp(preset, "standard") == 0) {
        *opts = DEFAULT_REPORT_OPTIONS;
        opts->target_segments = true;
        opts->vma_details = true;
        opts->fields = true;
        return true;
    }
    if (strcmp(preset, "full") == 0) {
        *opts = FULL_REPORT_OPTIONS;
        return true;
    }
    return false;
}
...
static bool is_bss_section_name(const char *name)
{
    return strcmp(name, ".bss") == 0 || strcmp(name, ".tbss") == 0;
}
...
static bool is_special_path(const char *path)
{
    return starts_with(path, "[vdso]") || starts_with(path, "[vvar]") ||
           starts_with(path, "[vsyscall]") || starts_with(path, "[rollup]");
}
...
static bool is_heap_path(const char *path)
{
    ...
    if (!starts_with(path, "[anon:")) {
        return false;
    }
    return strstr(path, "heap") != NULL || strstr(path, "jemalloc") != NULL ||
           strstr(path, "malloc") != NULL;
}
...
static void runtime_section_list_free(RuntimeSectionList *list)
{
    free(list->items);
    list->items = NULL;
    list->count = 0;
...
static bool runtime_section_list_push(RuntimeSectionList *list, const RuntimeSection *section)
{
    ...
    if (list->count == list->cap) {
        ...
        RuntimeSection *new_items = realloc(list->items, new_cap * sizeof(*new_items));
        if (new_items == NULL) {
            return false;
        }
        list->items = new_items;
...
static void clean_deleted_suffix(const char *in, char *out, size_t len)
{
    ...
    if (n > dlen && strcmp(in + n - dlen, deleted) == 0) {
        n -= dlen;
    }
    if (n >= len) {
        n = len - 1;
...
static void make_fallback_section_label(const Vma *v, char *label, size_t label_len,
                                        char *object, size_t object_len,
                                        char *name, size_t name_len)
{
    const char *seg = segment_name(v->segment);

    snprintf(object, object_len, "-");
    snprintf(name, name_len, "%s", seg);
    if (v->segment == SEG_HEAP) {
        snprintf(label, label_len, "heap");
    } else if (v->segment == SEG_STACK) {
        snprintf(label, label_len, "stack");
    } else if (v->segment == SEG_ANON) {
        snprintf(label, label_len, "anon");
    } else if (v->segment == SEG_SPECIAL) {
        snprintf(label, label_len, "special");
    } else if (v->segment == SEG_FILE) {
        snprintf(label, label_len, "file");
    } else {
        snprintf(label, label_len, "%s", seg);
    }
}

static bool elf_path_already_loaded(const RuntimeSectionList *sections, const char *path)
{
    for (size_t i = 0; i < sections->count; i++) {
        if (path_equal_ignoring_deleted(sections->items[i].path, path)) {
            return true;
        }
    }
    return false;
...
static bool load_elf_sections_for_path(const char *raw_path, const VmaList *list,
                                       long page_size, RuntimeSectionList *sections)
{
    char path[PATH_MAX];
    FILE *fp;
    unsigned char eh[64];
    uint64_t phoff, shoff, min_vaddr = UINT64_MAX, first_map_start = UINT64_MAX;
    uint16_t phnum, phentsize, shnum, shentsize, shstrndx;
    uint64_t load_bias;
    ElfLoadInfo loads[128];
    size_t load_count = 0;
    unsigned char *shdrs = NULL;
    char *shstr = NULL;
    uint64_t shstr_off, shstr_size;
    char object[SECTION_NAME_LEN];

    clean_deleted_suffix(raw_path, path, sizeof(path));
    if (path[0] != '/' || elf_path_already_loaded(sections, path)) {
        return true;
    }
    ...
    phnum = rd16(eh + 56);
    shentsize = rd16(eh + 58);
    shnum = rd16(eh + 60);
    shstrndx = rd16(eh + 62);

    if (phentsize < 56 || phnum == 0 || shoff == 0 ||
        shentsize < 64 || shnum == 0 || shstrndx >= shnum) {
        fclose(fp);
        return true;
    }

    if (fseeko(fp, (off_t)phoff, SEEK_SET) != 0) {
        fclose(fp);
        return true;
    }
    for (uint16_t i = 0; i < phnum; i++) {
        ...
        if (load_count < sizeof(loads) / sizeof(loads[0])) {
            loads[load_count].offset = rd64(ph + 8);
            loads[load_count].vaddr = rd64(ph + 16);
            loads[load_count].filesz = rd64(ph + 32);
            loads[load_count].memsz = rd64(ph + 40);
            load_count++;
        }
        uint64_t vaddr = rd64(ph + 16);
        if (vaddr < min_vaddr) {
            min_vaddr = page_align_down(vaddr, page_size);
        }
    }
    ...
    for (size_t i = 0; i < list->count; i++) {
        if (path_equal_ignoring_deleted(list->items[i].path, path) &&
    ...
    load_bias = first_map_start - min_vaddr;

    for (size_t i = 0; i < list->count; i++) {
        const Vma *v = &list->items[i];
        if (!path_equal_ignoring_deleted(v->path, path)) {
            continue;
        }
        for (size_t j = 0; j < load_count; j++) {
            const ElfLoadInfo *load = &loads[j];
            uint64_t file_start = page_align_down(load->offset, page_size);
            uint64_t file_end = load->offset + load->filesz;
            uint64_t mem_end = load->offset + load->memsz;
            if (file_end < mem_end) {
                file_end = mem_end;
            }
            if (v->offset < file_start || v->offset >= file_end) {
                continue;
            }
            uint64_t map_delta = v->offset >= load->offset ? v->offset - load->offset : 0;
            load_bias = v->start - (load->vaddr + map_delta);
            i = list->count;
            break;
        }
    }

    shdrs = malloc((size_t)shnum * shentsize);
    if (shdrs == NULL) {
        fclose(fp);
        return false;
    }
    if (fseeko(fp, (off_t)shoff, SEEK_SET) != 0 ||
        fread(shdrs, shentsize, shnum, fp) != shnum) {
        free(shdrs);
        fclose(fp);
        return true;
    }

    shstr_off = rd64(shdrs + (size_t)shstrndx * shentsize + 24);
    shstr_size = rd64(shdrs + (size_t)shstrndx * shentsize + 32);
    if (shstr_size == 0 || shstr_size > 64ULL * 1024ULL * 1024ULL) {
        free(shdrs);
        fclose(fp);
        return true;
    }
    shstr = malloc((size_t)shstr_size);
    if (shstr == NULL) {
        free(shdrs);
        fclose(fp);
        return false;
    }
    if (fseeko(fp, (off_t)shstr_off, SEEK_SET) != 0 ||
        fread(shstr, 1, (size_t)shstr_size, fp) != (size_t)shstr_size) {
        free(shstr);
        free(shdrs);
        fclose(fp);
        return true;
    ...
    snprintf(object, sizeof(object), "%s", path_basename_const(path));
    for (uint16_t i = 0; i < shnum; i++) {
        unsigned char *sh = shdrs + (size_t)i * shentsize;
        uint32_t sh_name = rd32(sh);
        uint64_t sh_flags = rd64(sh + 8);
        uint64_t sh_addr = rd64(sh + 16);
        uint64_t sh_size = rd64(sh + 32);
        RuntimeSection sec;
        const char *section_name;

        if ((sh_flags & 0x2ULL) == 0 || sh_size == 0 || sh_name >= shstr_size) {
            continue;
        }
        section_name = shstr + sh_name;
        if (section_name[0] == '\0') {
            continue;
        }

        memset(&sec, 0, sizeof(sec));
        snprintf(sec.path, sizeof(sec.path), "%s", path);
        snprintf(sec.object, sizeof(sec.object), "%s:%s", object, section_name);
        sec.start = load_bias + sh_addr;
        sec.end = sec.start + sh_size;
        sec.valid = true;
        if (!runtime_section_list_push(sections, &sec)) {
            free(shstr);
            free(shdrs);
            fclose(fp);
            return false;
...
static bool build_runtime_sections(const VmaList *list, long page_size,
                                   RuntimeSectionList *sections)
{
    for (size_t i = 0; i < list->count; i++) {
        const Vma *v = &list->items[i];
        if (v->path[0] == '\0' || v->path[0] == '[' || is_special_path(v->path)) {
            continue;
        }
        if (!load_elf_sections_for_path(v->path, list, page_size, sections)) {
            return false;
        }
    }
    return true;
...
{
    if (value <= start || value >= end || *count >= MAX_REGION_CUTS) {
        return;
    }
    cuts[(*count)++] = value;
}

static uint64_t overlap_len(uint64_t a_start, uint64_t a_end, uint64_t b_start, uint64_t b_end)
{
...
static const RuntimeSection *find_best_section(const RuntimeSectionList *sections,
                                               const Vma *v, uint64_t start, uint64_t end)
{
    const RuntimeSection *best = NULL;
    uint64_t best_overlap = 0;

    for (size_t i = 0; i < sections->count; i++) {
        const RuntimeSection *sec = &sections->items[i];
        uint64_t n;
        if (!path_equal_ignoring_deleted(v->path, sec->path)) {
            continue;
        }
        n = overlap_len(start, end, sec->start, sec->end);
        if (n > best_overlap || (n == best_overlap && n > 0 && best != NULL && sec->start < best->start)) {
            best = sec;
            best_overlap = n;
        }
    }
...
static void split_section_label(const char *label,
                                char *object, size_t object_len,
                                char *name, size_t name_len)
{
    ...
    size_t n = (size_t)(colon - label);
    if (n >= object_len) {
        n = object_len - 1;
    }
    memcpy(object, label, n);
    object[n] = '\0';
    snprintf(name, name_len, "%s", colon + 1);
}
...
static void assign_logical_region_identity(LogicalRegion *region, const char *exe_path,
                                           const ExeLayout *layout,
                                           const RuntimeSectionList *sections)
{
    const RuntimeSection *sec = find_best_section(sections, region->vma,
                                                  region->start, region->end);
    region->segment = classify_logical_region(region->vma, exe_path, layout,
                                              region->start, region->end);
    if (sec != NULL) {
        snprintf(region->section_label, sizeof(region->section_label), "%s", sec->object);
        split_section_label(region->section_label,
                            region->section_object, sizeof(region->section_object),
                            region->section_name, sizeof(region->section_name));
        if (is_bss_section_name(region->section_name) && region->segment != SEG_BSS) {
            snprintf(region->section_name, sizeof(region->section_name), ".data");
            snprintf(region->section_label, sizeof(region->section_label),
                     "%s:%s", region->section_object, region->section_name);
        }
        return;
    }
    make_fallback_section_label(region->vma,
                                region->section_label, sizeof(region->section_label),
                                region->section_object, sizeof(region->section_object),
                                region->section_name, sizeof(region->section_name));
}
...
static bool build_logical_regions(const VmaList *list, const char *exe_path,
                                  ...
                                  const RuntimeSectionList *sections,
                                  long page_size,
                                  LogicalRegionList *regions)
{
    for (size_t i = 0; i < list->count; i++) {
        const Vma *v = &list->items[i];
        uint64_t cuts[MAX_REGION_CUTS];
        size_t cut_count = 0;
        ...
        for (size_t j = 0; j < sections->count; j++) {
            const RuntimeSection *sec = &sections->items[j];
            if (!path_equal_ignoring_deleted(v->path, sec->path) ||
                !overlaps(v->start, v->end, sec->start, sec->end)) {
                continue;
            }
            add_cut(cuts, &cut_count, page_align_down(sec->start, page_size), v->start, v->end);
            add_cut(cuts, &cut_count, page_align_up(sec->end, page_size), v->start, v->end);
        }
        ...
        for (size_t j = 1; j < cut_count; j++) {
            ...
            region.end = cuts[j];
            assign_logical_region_identity(&region, exe_path, layout, sections);
...
static void update_page_stats_from_code(PageStats *ps, unsigned char code, uint64_t pfn)
{
    ps->virtual_pages++;
    if ((code & PAGE_CODE_PRESENT) != 0) {
        ps->present_pages++;
        if (pfn != 0) {
            if (!ps->has_pfn) {
                ps->pfn_min = pfn;
    ...
    } else if ((code & PAGE_CODE_SWAPPED) != 0) {
        ps->swapped_pages++;
    } else {
        ps->not_present_pages++;
    }
    if ((code & PAGE_CODE_SOFT_DIRTY) != 0) ps->soft_dirty_pages++;
    if ((code & PAGE_CODE_EXCLUSIVE) != 0) ps->exclusive_pages++;
    if ((code & PAGE_CODE_FILE_SHARED) != 0) ps->file_or_shared_pages++;
}

static void derive_logical_pagemap_from_vmas(LogicalRegionList *regions, long page_size)
{
    for (size_t i = 0; i < regions->count; i++) {
        LogicalRegion *r = &regions->items[i];
        const Vma *v = r->vma;
        PageStats *ps = &r->pages;
        size_t first = (size_t)((r->start - v->start) / (uint64_t)page_size);
        size_t count = (size_t)((r->end - r->start) / (uint64_t)page_size);
        ...
        for (size_t p = 0; p < count; p++) {
            update_page_stats_from_code(ps, v->page_codes[first + p],
                                        v->page_pfns != NULL ? v->page_pfns[first + p] : 0);
        }
    }
}
...
static void accumulate_summary(const LogicalRegionList *regions, long page_size, SegmentSummary sums[SEG_COUNT])
{
    (void)page_size;
    for (int s = 0; s < SEG_COUNT; s++) {
        sums[s].pfn_min = UINT64_MAX;
        sums[s].addr_min = UINT64_MAX;
    }
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        sums[s].vma_count++;
        if (r->start < sums[s].addr_min) sums[s].addr_min = r->start;
        if (r->end > sums[s].addr_max) sums[s].addr_max = r->end;
        if (strchr(v->perms, 'r') != NULL) sums[s].perm_mask |= 1U;
        if (strchr(v->perms, 'w') != NULL) sums[s].perm_mask |= 2U;
        if (strchr(v->perms, 'x') != NULL) sums[s].perm_mask |= 4U;
        if (strchr(v->perms, 's') != NULL) sums[s].perm_mask |= 8U;
        if (strchr(v->perms, 'p') != NULL) sums[s].perm_mask |= 16U;
        if (is_special_path(v->path)) {
            sums[s].special_sources++;
        } else if (v->path[0] != '\0') {
            sums[s].file_sources++;
        } else {
            sums[s].anon_sources++;
        ...
        if (r->pages.has_pfn) {
            if (!sums[s].has_pfn) {
                sums[s].pfn_min = r->pages.pfn_min;
                sums[s].pfn_max = r->pages.pfn_max;
                sums[s].has_pfn = true;
            } else {
                if (r->pages.pfn_min < sums[s].pfn_min) sums[s].pfn_min = r->pages.pfn_min;
                if (r->pages.pfn_max > sums[s].pfn_max) sums[s].pfn_max = r->pages.pfn_max;
            }
            sums[s].pfn_runs += r->pages.pfn_runs;
        }
        ...
        sums[s].swappss_kb += v->swappss_kb >= 0 ? prorate_kb(v->swappss_kb, part_bytes, total_bytes) : 0;
        sums[s].referenced_kb += v->referenced_kb >= 0 ? prorate_kb(v->referenced_kb, part_bytes, total_bytes) : 0;
    }
}
...
static void section_summary_list_free(SectionSummaryList *list)
{
    free(list->items);
    list->items = NULL;
    list->count = 0;
...
static SectionSummaryRow *find_or_add_section_summary(SectionSummaryList *list, SegmentKind segment, const char *label)
{
    for (size_t i = 0; i < list->count; i++) {
        if (list->items[i].segment == segment && strcmp(list->items[i].label, label) == 0) {
            return &list->items[i];
        }
    }
    if (list->count == list->cap) {
        size_t new_cap = list->cap == 0 ? 64 : list->cap * 2;
        SectionSummaryRow *items = realloc(list->items, new_cap * sizeof(*items));
        if (items == NULL) {
            return NULL;
        }
        list->items = items;
    ...
    SectionSummaryRow *row = &list->items[list->count++];
    memset(row, 0, sizeof(*row));
    row->segment = segment;
    snprintf(row->label, sizeof(row->label), "%s", label[0] ? label : "unknown");
    return row;
}

static int compare_section_summary_rows(const void *a, const void *b)
{
    const SectionSummaryRow *ra = (const SectionSummaryRow *)a;
    const SectionSummaryRow *rb = (const SectionSummaryRow *)b;
    if (ra->segment != rb->segment) return (int)ra->segment - (int)rb->segment;
    if (ra->sums.size_kb < rb->sums.size_kb) return 1;
    if (ra->sums.size_kb > rb->sums.size_kb) return -1;
    return strcmp(ra->label, rb->label);
}

static void print_section_summary(FILE *out, const LogicalRegionList *regions, long page_size)
{
    SectionSummaryList list = {0};
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        uint64_t part_bytes = r->end - r->start;
        uint64_t total_bytes = v->end - v->start;
        SegmentKind seg = r->segment;
        if (seg < 0 || seg >= SEG_COUNT) {
            seg = SEG_UNKNOWN;
        }
        SectionSummaryRow *row = find_or_add_section_summary(&list, seg, r->section_label);
        if (row == NULL) {
            section_summary_list_free(&list);
            fprintf(out, "## Section 汇总\n\n内存不足，无法生成 section 汇总。\n\n");
            return;
        }
        row->sums.virtual_pages += r->pages.virtual_pages;
        row->sums.present_pages += r->pages.present_pages;
        row->sums.swapped_pages += r->pages.swapped_pages;
        row->sums.not_present_pages += r->pages.not_present_pages;
        row->sums.size_kb += region_size_kb(r);
        row->sums.rss_kb += v->rss_kb >= 0 ? prorate_kb(v->rss_kb, part_bytes, total_bytes) : 0;
        row->sums.pss_kb += v->pss_kb >= 0 ? prorate_kb(v->pss_kb, part_bytes, total_bytes) : 0;
        row->sums.swap_kb += v->swap_kb >= 0 ? prorate_kb(v->swap_kb, part_bytes, total_bytes) : 0;
    }

    qsort(list.items, list.count, sizeof(list.items[0]), compare_section_summary_rows);
    fprintf(out, "## Section 细分汇总\n\n");
    fprintf(out, "> 一级分类仍使用 v2 的 segment；二级分类在 ELF 映射中使用 section header 的 `sh_name`，无法匹配 section 的区域显示为对应 segment 的 fallback 名称。\n\n");
    fprintf(out, "| 一级段 | 二级 section | Size(KiB) | Rss(KiB) | Present(KiB) | NotPresent(KiB) | Present%% |\n");
    fprintf(out, "| --- | --- | ---: | ---: | ---: | ---: | ---: |\n");
    for (size_t i = 0; i < list.count; i++) {
        const SectionSummaryRow *row = &list.items[i];
        double pct = row->sums.virtual_pages == 0 ? 0.0 :
                     100.0 * (double)row->sums.present_pages / (double)row->sums.virtual_pages;
        fprintf(out, "| %s | `", segment_name(row->segment));
        print_md_text_cell(out, row->label);
        fprintf(out, "` | %ld | %ld | %llu | %llu | %.2f%% |\n",
                row->sums.size_kb,
                row->sums.rss_kb,
                row->sums.present_pages * (unsigned long long)page_size / 1024ULL,
                row->sums.not_present_pages * (unsigned long long)page_size / 1024ULL,
                pct);
    }
    fprintf(out, "\n");
    section_summary_list_free(&list);
...
static void __attribute__((unused)) print_process_summary(FILE *out, const VmaList *list, long page_size)
{
...
static void accumulate_total_overview(const VmaList *list, SegmentSummary *sum)
{
    memset(sum, 0, sizeof(*sum));
    sum->addr_min = UINT64_MAX;
    sum->pfn_min = UINT64_MAX;

    for (size_t i = 0; i < list->count; i++) {
        const Vma *v = &list->items[i];
        sum->vma_count++;
        if (v->start < sum->addr_min) sum->addr_min = v->start;
        if (v->end > sum->addr_max) sum->addr_max = v->end;
        if (strchr(v->perms, 'r') != NULL) sum->perm_mask |= 1U;
        if (strchr(v->perms, 'w') != NULL) sum->perm_mask |= 2U;
        if (strchr(v->perms, 'x') != NULL) sum->perm_mask |= 4U;
        if (strchr(v->perms, 's') != NULL) sum->perm_mask |= 8U;
        if (strchr(v->perms, 'p') != NULL) sum->perm_mask |= 16U;
        if (is_special_path(v->path)) {
            sum->special_sources++;
        } else if (v->path[0] != '\0') {
            sum->file_sources++;
        ...
        if (v->size_kb >= 0) sum->size_kb += v->size_kb;
        if (v->rss_kb >= 0) sum->rss_kb += v->rss_kb;
        if (v->pss_kb >= 0) sum->pss_kb += v->pss_kb;
        if (v->swap_kb >= 0) sum->swap_kb += v->swap_kb;
        if (v->swappss_kb >= 0) sum->swappss_kb += v->swappss_kb;
        if (v->referenced_kb >= 0) sum->referenced_kb += v->referenced_kb;
        sum->virtual_pages += v->pages.virtual_pages;
        sum->present_pages += v->pages.present_pages;
        sum->swapped_pages += v->pages.swapped_pages;
        sum->not_present_pages += v->pages.not_present_pages;
        if (v->pages.has_pfn) {
            if (!sum->has_pfn) {
                sum->pfn_min = v->pages.pfn_min;
                sum->pfn_max = v->pages.pfn_max;
                sum->has_pfn = true;
            } else {
                if (v->pages.pfn_min < sum->pfn_min) sum->pfn_min = v->pages.pfn_min;
                if (v->pages.pfn_max > sum->pfn_max) sum->pfn_max = v->pages.pfn_max;
            }
            sum->pfn_runs += v->pages.pfn_runs;
        }
    }
}

static void print_summary_perm_cell(FILE *out, const SegmentSummary *s)
{
    if (s->vma_count == 0) {
        fprintf(out, "-");
        return;
    }
    fputc((s->perm_mask & 1U) ? 'r' : '-', out);
    fputc((s->perm_mask & 2U) ? 'w' : '-', out);
    fputc((s->perm_mask & 4U) ? 'x' : '-', out);
    if ((s->perm_mask & 8U) && (s->perm_mask & 16U)) {
        fprintf(out, " p/s");
    } else if (s->perm_mask & 8U) {
        fprintf(out, " s");
    } else if (s->perm_mask & 16U) {
        fprintf(out, " p");
    }
}

static void print_summary_source_cell(FILE *out, const SegmentSummary *s)
{
    if (s->vma_count == 0) {
        fprintf(out, "-");
        return;
    }
    fprintf(out, "file:%zu anon:%zu special:%zu",
            s->file_sources, s->anon_sources, s->special_sources);
}

static void print_summary_addr_cell(FILE *out, const SegmentSummary *s)
{
    if (s->vma_count == 0 || s->addr_min == UINT64_MAX) {
        fprintf(out, "-");
    } else {
        fprintf(out, "`%012" PRIx64 "-%012" PRIx64 "`", s->addr_min, s->addr_max);
    }
}

static void print_summary_percent_cell(FILE *out, const SegmentSummary *s)
{
    if (s->virtual_pages == 0) {
        fprintf(out, "-");
    } else {
        fprintf(out, "%.2f%%", 100.0 * (double)s->present_pages / (double)s->virtual_pages);
    }
}

static void print_overview_metric_cell(FILE *out, const SegmentSummary *s,
                                       const char *metric, long page_size)
{
    if (strcmp(metric, "vma_count") == 0) fprintf(out, "%zu", s->vma_count);
    else if (strcmp(metric, "addr") == 0) print_summary_addr_cell(out, s);
    else if (strcmp(metric, "perms") == 0) print_summary_perm_cell(out, s);
    else if (strcmp(metric, "source") == 0) print_summary_source_cell(out, s);
    else if (strcmp(metric, "inode") == 0) fprintf(out, "%zu", s->inode_sources);
    else if (strcmp(metric, "size") == 0) fprintf(out, "%ld KiB", s->size_kb);
    else if (strcmp(metric, "rss") == 0) fprintf(out, "%ld KiB", s->rss_kb);
    else if (strcmp(metric, "pss") == 0) fprintf(out, "%ld KiB", s->pss_kb);
    else if (strcmp(metric, "swap") == 0) fprintf(out, "%ld KiB", s->swap_kb);
    else if (strcmp(metric, "swappss") == 0) fprintf(out, "%ld KiB", s->swappss_kb);
    else if (strcmp(metric, "referenced") == 0) fprintf(out, "%ld KiB", s->referenced_kb);
    else if (strcmp(metric, "virtual_pages") == 0) fprintf(out, "%llu", s->virtual_pages);
    else if (strcmp(metric, "present_pages") == 0) fprintf(out, "%llu / %llu KiB", s->present_pages, s->present_pages * (unsigned long long)page_size / 1024ULL);
    else if (strcmp(metric, "swap_pages") == 0) fprintf(out, "%llu", s->swapped_pages);
    else if (strcmp(metric, "not_present_pages") == 0) fprintf(out, "%llu", s->not_present_pages);
    else if (strcmp(metric, "present_pct") == 0) print_summary_percent_cell(out, s);
    else if (strcmp(metric, "pfn_min") == 0) s->has_pfn ? fprintf(out, "%" PRIu64, s->pfn_min) : fprintf(out, "-");
    else if (strcmp(metric, "pfn_max") == 0) s->has_pfn ? fprintf(out, "%" PRIu64, s->pfn_max) : fprintf(out, "-");
    else if (strcmp(metric, "pfn_runs") == 0) fprintf(out, "%llu", s->pfn_runs);
    else fprintf(out, "-");
}

static void print_overview_matrix(FILE *out, const VmaList *list,
                                  const LogicalRegionList *regions, long page_size)
{
    static const SegmentKind cols[] = {
        SEG_HEAP, SEG_TEXT, SEG_INIT_DATA, SEG_BSS,
        SEG_STACK, SEG_FILE, SEG_ANON, SEG_SPECIAL
    };
    static const struct {
        const char *group;
        const char *label;
        ...
        {"maps", "逻辑区间数", "vma_count"},
        {"maps", "地址范围", "addr"},
        {"maps", "权限", "perms"},
        {"maps", "路径/来源", "source"},
        {"maps", "文件 inode", "inode"},
        {"smaps", "虚拟地址空间 Size", "size"},
        {"smaps", "RSS", "rss"},
        {"smaps", "PSS", "pss"},
        {"smaps", "Swap", "swap"},
        {"smaps", "SwapPSS", "swappss"},
        {"smaps", "Referenced", "referenced"},
        {"pagemap", "虚拟页", "virtual_pages"},
        {"pagemap", "present页", "present_pages"},
        {"pagemap", "swap页", "swap_pages"},
        {"pagemap", "未驻留页", "not_present_pages"},
        {"pagemap", "present占比", "present_pct"},
        {"pagemap", "PFN min", "pfn_min"},
        {"pagemap", "PFN max", "pfn_max"},
        {"pagemap", "连续区段数", "pfn_runs"},
    };
    SegmentSummary total;
    SegmentSummary sums[SEG_COUNT];

    memset(sums, 0, sizeof(sums));
    accumulate_total_overview(list, &total);
    accumulate_summary(regions, page_size, sums);
    total.vma_count = regions->count;

    fprintf(out, "## 总览矩阵\n\n");
    fprintf(out, "> 列为总进程与一级 segment；行为 `maps`、`smaps`、`pagemap` 指标。`路径/来源` 显示 file/anon/special 来源计数；分段列中的 `逻辑区间数` 是报告内部按 ELF/section 边界切分后的区间数量。\n\n");
    fprintf(out, "| 来源 | 指标 | 总进程");
    for (size_t c = 0; c < sizeof(cols) / sizeof(cols[0]); c++) {
        fprintf(out, " | %s", segment_name(cols[c]));
    }
    fprintf(out, " |\n| --- | --- | ---: |");
    for (size_t c = 0; c < sizeof(cols) / sizeof(cols[0]); c++) {
        fprintf(out, " ---: |");
    ...
    for (size_t r = 0; r < sizeof(rows) / sizeof(rows[0]); r++) {
        fprintf(out, "| %s | %s | ", rows[r].group, rows[r].label);
        print_overview_metric_cell(out, &total, rows[r].metric, page_size);
        for (size_t c = 0; c < sizeof(cols) / sizeof(cols[0]); c++) {
            fprintf(out, " | ");
            print_overview_metric_cell(out, &sums[cols[c]], rows[r].metric, page_size);
        }
        fprintf(out, " |\n");
    }
    fprintf(out, "\n");
...
static void __attribute__((unused)) print_segment_summary(FILE *out, const LogicalRegionList *regions, long page_size)
{
    SegmentSummary sums[SEG_COUNT];
    memset(sums, 0, sizeof(sums));
    accumulate_summary(regions, page_size, sums);
...
static void print_vma_details(FILE *out, const LogicalRegionList *regions, long page_size)
{
    ...
    fprintf(out, "| 逻辑地址范围 | 原始 VMA | 一级段 | 二级 section | 权限 | Size(KiB) | Rss(KiB) | Pss(KiB) | Swap(KiB) | Present(KiB) | NotPresent(KiB) | 路径 |\n");
    fprintf(out, "| --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |\n");
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        uint64_t part_bytes = r->end - r->start;
        uint64_t total_bytes = v->end - v->start;
        fprintf(out, "| `%012" PRIx64 "-%012" PRIx64 "` | `%012" PRIx64 "-%012" PRIx64 "` | %s | `",
                r->start, r->end,
                v->start, v->end,
                segment_name(r->segment));
        print_md_text_cell(out, r->section_label);
        fprintf(out, "` | `%s` | %ld | %ld | %ld | %ld | %llu | %llu | `",
                v->perms,
                region_size_kb(r),
...
static void print_logical_pfn_sequences(FILE *out, const LogicalRegionList *regions, long page_size)
{
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        fprintf(out, "### `%012" PRIx64 "-%012" PRIx64 "` %s / %s\n\n",
                r->start, r->end, segment_name(r->segment), r->section_label);
        fprintf(out, "- 一级段：%s\n", segment_name(r->segment));
        fprintf(out, "- 二级 section：`");
        print_md_text_cell(out, r->section_label);
        fprintf(out, "`\n");
...
static void print_field_explanations(FILE *out)
{
    ...
    fprintf(out, "| `heap/堆` | `pathname` 为 `[heap]`，或 `[anon:*]` 名称中包含 `heap`、`jemalloc`、`malloc` 的分配器/运行时堆映射。 |\n");
...
static void split_writer_close_part(SplitWriter *w)
{
    if (w->part != NULL) {
        fclose(w->part);
        w->part = NULL;
...
static bool split_writer_open_part(SplitWriter *w, const char *title)
{
    char path[PATH_MAX];
    char link[PATH_MAX];

    split_writer_close_part(w);
    snprintf(w->title, sizeof(w->title), "%s", title);
    w->part_no++;
    if (snprintf(path, sizeof(path), "%s/part_%03d.md", w->dir, w->part_no) >= (int)sizeof(path) ||
        snprintf(link, sizeof(link), "%s/part_%03d.md", w->link_dir, w->part_no) >= (int)sizeof(link)) {
        errno = ENAMETOOLONG;
        return false;
    }
    w->part = fopen(path, "w");
    ...
    fprintf(w->part, "# %s\n\n", title);
    fprintf(w->index, "- [%s part %03d](%s)\n", title, w->part_no, link);
    return true;
}

static bool split_writer_rotate_if_needed(SplitWriter *w)
{
    long pos;
    if (w->part == NULL) {
        return false;
    ...
    pos = ftell(w->part);
    if (pos < 0 || pos < DETAIL_ROTATE_LIMIT) {
        return true;
    }
    return split_writer_open_part(w, w->title);
}

static bool __attribute__((unused)) split_writer_printf(SplitWriter *w, const char *fmt, ...)
{
    va_list ap;
    if (w->part == NULL) {
        return false;
    ...
    va_start(ap, fmt);
    vfprintf(w->part, fmt, ap);
    va_end(ap);
    return split_writer_rotate_if_needed(w);
}

static bool print_split_target_segments(SplitWriter *w, const LogicalRegionList *regions, long page_size)
{
    if (!split_writer_open_part(w, "重点分段字段")) return false;
    fprintf(w->part, "下面只列出图中关心的 `text`、`data`、`bss`、`heap`、`file`、`stack`。`Present(KiB)` 是真正驻留在物理内存中的页大小。\n\n");
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        fprintf(w->part, "## %s / %s\n\n", segment_name(r->segment), r->section_label);
        fprintf(w->part, "- 逻辑地址范围：`%012" PRIx64 "-%012" PRIx64 "`\n", r->start, r->end);
        fprintf(w->part, "- 原始 VMA：`%012" PRIx64 "-%012" PRIx64 "`\n", v->start, v->end);
        fprintf(w->part, "- 权限：`%s`\n", v->perms);
        fprintf(w->part, "- 路径：`");
        print_md_text_cell(w->part, display_path(v));
        fprintf(w->part, "`\n");
        fprintf(w->part, "- maps：`offset=0x%" PRIx64 "`, `dev=%s`, `inode=%llu`\n",
                v->offset, v->dev, v->inode);
        fprintf(w->part, "- smaps：`Size=");
        print_kb(w->part, region_size_kb(r));
        fprintf(w->part, " KiB`, `Rss=");
        print_kb(w->part, prorate_kb(v->rss_kb, part_bytes, total_bytes));
        fprintf(w->part, " KiB`, `Pss=");
        print_kb(w->part, prorate_kb(v->pss_kb, part_bytes, total_bytes));
        fprintf(w->part, " KiB`, `Referenced=");
        print_kb(w->part, prorate_kb(v->referenced_kb, part_bytes, total_bytes));
        fprintf(w->part, " KiB`, `Swap=");
        print_kb(w->part, prorate_kb(v->swap_kb, part_bytes, total_bytes));
        fprintf(w->part, " KiB`, `SwapPss=");
        print_kb(w->part, prorate_kb(v->swappss_kb, part_bytes, total_bytes));
        fprintf(w->part, " KiB`\n");
        fprintf(w->part, "- pagemap：`virtual=%llu页`, `present=%llu页(%llu KiB)`, `not_present=%llu页(%llu KiB)`, `swapped=%llu页`, `soft_dirty=%llu页`, `exclusive=%llu页`, `file_or_shared=%llu页`\n",
                r->pages.virtual_pages,
                r->pages.present_pages,
                r->pages.present_pages * (unsigned long long)page_size / 1024ULL,
                r->pages.not_present_pages,
                ...
                r->pages.file_or_shared_pages);
        if (r->pages.has_pfn) {
            fprintf(w->part, "- PFN：`min=%" PRIu64 "`, `max=%" PRIu64 "`, `连续区段数=%llu`\n",
                    r->pages.pfn_min, r->pages.pfn_max, r->pages.pfn_runs);
        } else if (r->pages.present_pages > 0) {
            fprintf(w->part, "- PFN：未显示，通常是因为没有 `root/CAP_SYS_ADMIN` 权限或内核隐藏 PFN。\n");
        }
        fprintf(w->part, "\n");
        if (!split_writer_rotate_if_needed(w)) return false;
    }
    return true;
}

static bool print_split_vma_details(SplitWriter *w, const LogicalRegionList *regions, long page_size)
{
    if (!split_writer_open_part(w, "VMA 明细（逻辑切分）")) return false;
    fprintf(w->part, "| 逻辑地址范围 | 原始 VMA | 一级段 | 二级 section | 权限 | Size(KiB) | Rss(KiB) | Pss(KiB) | Swap(KiB) | Present(KiB) | NotPresent(KiB) | 路径 |\n");
    fprintf(w->part, "| --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |\n");
    for (size_t i = 0; i < regions->count; i++) {
        const LogicalRegion *r = &regions->items[i];
        const Vma *v = r->vma;
        uint64_t part_bytes = r->end - r->start;
        uint64_t total_bytes = v->end - v->start;
        fprintf(w->part, "| `%012" PRIx64 "-%012" PRIx64 "` | `%012" PRIx64 "-%012" PRIx64 "` | %s | `",
                r->start, r->end, v->start, v->end, segment_name(r->segment));
        print_md_text_cell(w->part, r->section_label);
        fprintf(w->part, "` | `%s` | %ld | %ld | %ld | %ld | %llu | %llu | `",
                v->perms,
                region_size_kb(r),
                prorate_kb(v->rss_kb, part_bytes, total_bytes),
                prorate_kb(v->pss_kb, part_bytes, total_bytes),
...
static bool print_split_vma_page_bitmaps(SplitWriter *w, const VmaList *list, long page_size)
{
    const size_t chunk_pages = 8192;
    if (!split_writer_open_part(w, "VMA 页状态序列")) return false;
    fprintf(w->part, "每个字符对应一个虚拟页，按地址从低到高排列：`P=present`，`N=not present`，`S=swapped`，`?=读取失败`。\n\n");

    for (size_t i = 0; i < list->count; i++) {
        const Vma *v = &list->items[i];
        size_t page_count = (size_t)((v->end - v->start) / (uint64_t)page_size);
        size_t chunks = page_count == 0 ? 1 : (page_count + chunk_pages - 1) / chunk_pages;

        for (size_t chunk = 0; chunk < chunks; chunk++) {
            size_t begin = chunk * chunk_pages;
            size_t end = begin + chunk_pages;
            if (end > page_count) end = page_count;
            if (!split_writer_rotate_if_needed(w)) return false;
            fprintf(w->part, "## `%012" PRIx64 "-%012" PRIx64 "` %s",
                    v->start + (uint64_t)begin * (uint64_t)page_size,
                    v->start + (uint64_t)end * (uint64_t)page_size,
                    segment_name(v->segment));
            if (chunks > 1) fprintf(w->part, " chunk %zu/%zu", chunk + 1, chunks);
            fprintf(w->part, "\n\n");
            fprintf(w->part, "- 原始 VMA：`%012" PRIx64 "-%012" PRIx64 "`\n", v->start, v->end);
            fprintf(w->part, "- 权限：`%s`\n", v->perms);
            fprintf(w->part, "- 路径：`");
            print_md_text_cell(w->part, display_path(v));
            fprintf(w->part, "`\n");
            fprintf(w->part, "- pagemap_bitmap：\n\n```text\n");
            if (v->page_states == NULL) {
                fprintf(w->part, "(无法读取 pagemap)");
            } else {
                for (size_t p = begin; p < end; p++) {
                    char state = v->page_states[p];
                    fputc(state != '\0' ? state : '?', w->part);
                    if ((p - begin + 1) % 128 == 0 && p + 1 < end) {
                        fputc('\n', w->part);
                    }
                }
            }
...
static bool print_split_logical_pfn_sequences(SplitWriter *w, const LogicalRegionList *regions, long page_size)
{
    const size_t chunk_pages = 4096;
    if (!split_writer_open_part(w, "逻辑区间 PFN 序列")) return false;
    fprintf(w->part, "用于自动对比操作前后的物理页变化。`pagemap_bitmap`、`page_code_sequence`、`pfn_sequence` 一一对应，每个位置代表一个虚拟页；PFN 为 `0` 表示该页未驻留、被换出、读取失败，或当前内核没有向本进程暴露 PFN。\n\n");
    fprintf(w->part, "page_code 是位掩码：`0=not-present/无标志`, `1=present`, `2=soft-dirty`, `4=swapped`, `8=exclusive`, `16=file/shared`, `32=PFN可见`。\n\n");
    ...
    for (size_t i = 0; i < regions->count; i++) {
        ...
        size_t first = (size_t)((r->start - v->start) / (uint64_t)page_size);
        size_t count = (size_t)((r->end - r->start) / (uint64_t)page_size);
        size_t chunks = count == 0 ? 1 : (count + chunk_pages - 1) / chunk_pages;
        ...
        for (size_t chunk = 0; chunk < chunks; chunk++) {
            ...
            fprintf(w->part, "## `%012" PRIx64 "-%012" PRIx64 "` %s / %s",
                    r->start + (uint64_t)begin * (uint64_t)page_size,
                    r->start + (uint64_t)end * (uint64_t)page_size,
                    segment_name(r->segment), r->section_label);
            if (chunks > 1) fprintf(w->part, " chunk %zu/%zu", chunk + 1, chunks);
            fprintf(w->part, "\n\n");
            fprintf(w->part, "- 一级段：%s\n", segment_name(r->segment));
            fprintf(w->part, "- 二级 section：`");
            print_md_text_cell(w->part, r->section_label);
            fprintf(w->part, "`\n");
            fprintf(w->part, "- 原始逻辑区间：`%012" PRIx64 "-%012" PRIx64 "`\n", r->start, r->end);
            fprintf(w->part, "- 原始 VMA：`%012" PRIx64 "-%012" PRIx64 "`\n", v->start, v->end);
            fprintf(w->part, "- 权限：`%s`\n", v->perms);
            fprintf(w->part, "- 路径：`");
            print_md_text_cell(w->part, display_path(v));
            fprintf(w->part, "`\n");
            fprintf(w->part, "- pagemap：`virtual=%llu页`, `present=%llu页`, `not_present=%llu页`, `swapped=%llu页`\n",
                    r->pages.virtual_pages, r->pages.present_pages, r->pages.not_present_pages, r->pages.swapped_pages);

            fprintf(w->part, "- pagemap_bitmap：\n\n```text\n");
            for (size_t p = begin; p < end; p++) {
                char state = v->page_states != NULL ? v->page_states[first + p] : '?';
                fputc(state != '\0' ? state : '?', w->part);
                if ((p - begin + 1) % 128 == 0 && p + 1 < end) fputc('\n', w->part);
            }
            fprintf(w->part, "\n```\n");

            fprintf(w->part, "- page_code_sequence：\n\n```text\n");
            for (size_t p = begin; p < end; p++) {
                if (p > begin) fputc(' ', w->part);
                fprintf(w->part, "%u", v->page_codes != NULL ? (unsigned int)v->page_codes[first + p] : 0U);
                if ((p - begin + 1) % 64 == 0 && p + 1 < end) fputc('\n', w->part);
            }
            fprintf(w->part, "\n```\n");

            fprintf(w->part, "- pfn_sequence：\n\n```text\n");
            for (size_t p = begin; p < end; p++) {
                if (p > begin) fputc(' ', w->part);
                fprintf(w->part, "%" PRIx64, v->page_pfns != NULL ? v->page_pfns[first + p] : (uint64_t)0);
                if ((p - begin + 1) % 32 == 0 && p + 1 < end) fputc('\n', w->part);
            }
            fprintf(w->part, "\n```\n\n");
        }
    }
...
static bool write_page_data_tsv(const char *path, const LogicalRegionList *regions, long page_size)
{
    FILE *out = fopen(path, "w");
    if (out == NULL) {
        return false;
    }
    fprintf(out, "addr\tsegment\tsection\tstatus\tcode\tpfn\n");
    for (size_t i = 0; i < regions->count; i++) {
        const LogicalRegion *r = &regions->items[i];
        const Vma *v = r->vma;
        size_t first = (size_t)((r->start - v->start) / (uint64_t)page_size);
        size_t count = (size_t)((r->end - r->start) / (uint64_t)page_size);
        const char *seg = segment_name(r->segment);
        for (size_t p = 0; p < count; p++) {
            uint64_t addr = r->start + (uint64_t)p * (uint64_t)page_size;
            char status = v->page_states != NULL ? v->page_states[first + p] : '?';
            unsigned int code = v->page_codes != NULL ? (unsigned int)v->page_codes[first + p] : 0U;
            uint64_t pfn = v->page_pfns != NULL ? v->page_pfns[first + p] : (uint64_t)0;
            fprintf(out, "%012" PRIx64 "\t%s\t", addr, seg);
            print_md_text_cell(out, r->section_label);
            fprintf(out, "\t%c\t%u\t%" PRIx64 "\n", status != '\0' ? status : '?', code, pfn);
        }
    }
    fclose(out);
    return true;
...
static bool write_split_detail_report(const char *detail_dir, const char *detail_link,
                                      pid_t pid, const char *comm, const char *exe_path,
                                      const char *pagemap_status, long page_size,
                                      const VmaList *list, const LogicalRegionList *regions,
                                      const ReportOptions *opts)
{
    char index_path[PATH_MAX];
    char full_path[PATH_MAX];
    char page_data_path[PATH_MAX];
    SplitWriter w;
    FILE *fields = NULL;
    FILE *full = NULL;

    memset(&w, 0, sizeof(w));
    snprintf(w.dir, sizeof(w.dir), "%s", detail_dir);
    snprintf(w.link_dir, sizeof(w.link_dir), "%s", detail_link);
    if (!mkdir_p(w.dir)) return false;
    if (snprintf(index_path, sizeof(index_path), "%s/index.md", w.dir) >= (int)sizeof(index_path)) {
        errno = ENAMETOOLONG;
        return false;
    }

    if (snprintf(page_data_path, sizeof(page_data_path), "%s/page_data.tsv", w.dir) >= (int)sizeof(page_data_path)) {
        errno = ENAMETOOLONG;
        return false;
    }
    if (!write_page_data_tsv(page_data_path, regions, page_size)) {
        return false;
    }

    if (opts->full_report) {
        if (snprintf(full_path, sizeof(full_path), "%s/full_report.md", w.dir) >= (int)sizeof(full_path)) {
            errno = ENAMETOOLONG;
            return false;
        }
        full = fopen(full_path, "w");
        if (full == NULL) return false;
        print_report_header(full, pid, comm, exe_path, pagemap_status, page_size);
        print_overview_matrix(full, list, regions, page_size);
        print_section_summary(full, regions, page_size);
        if (opts->target_segments) print_target_segments(full, regions, page_size);
        if (opts->vma_details) print_vma_details(full, regions, page_size);
        if (opts->vma_page_bitmaps) print_vma_page_bitmaps(full, list, page_size);
        if (opts->logical_pfn_sequences) print_logical_pfn_sequences(full, regions, page_size);
        if (opts->fields) print_field_explanations(full);
        fclose(full);
        full = NULL;
    }

    w.index = fopen(index_path, "w");
    if (w.index == NULL) return false;

    print_report_header(w.index, pid, comm, exe_path, pagemap_status, page_size);
    print_overview_matrix(w.index, list, regions, page_size);
    print_section_summary(w.index, regions, page_size);
    fprintf(w.index, "## 详细章节\n\n");
    fprintf(w.index, "- [机器可读逐页数据](page_data.tsv)\n");
    if (opts->full_report) {
        fprintf(w.index, "- [完整报告](full_report.md)\n");
    }

    if (opts->target_segments && !print_split_target_segments(&w, regions, page_size)) {
        split_writer_close_part(&w);
        fclose(w.index);
        return false;
    }
    if (opts->vma_details && !print_split_vma_details(&w, regions, page_size)) {
        split_writer_close_part(&w);
        fclose(w.index);
        return false;
    }
    if (opts->vma_page_bitmaps && !print_split_vma_page_bitmaps(&w, list, page_size)) {
        split_writer_close_part(&w);
        fclose(w.index);
        return false;
    }
    if (opts->logical_pfn_sequences && !print_split_logical_pfn_sequences(&w, regions, page_size)) {
        split_writer_close_part(&w);
        fclose(w.index);
        return false;
    }
    ...
    if (opts->fields) {
        fprintf(w.index, "- [字段含义](fields.md)\n");
    }
    fclose(w.index);
    w.index = NULL;
    ...
    if (snprintf(index_path, sizeof(index_path), "%s/fields.md", w.dir) >= (int)sizeof(index_path)) {
        errno = ENAMETOOLONG;
        return false;
    }
    fields = fopen(index_path, "w");
    if (fields == NULL) return false;
    print_field_explanations(fields);
    fclose(fields);
    return true;
}
...
static bool analyze_one_pid(pid_t pid, FILE *out)
{
    ...
    if (!build_runtime_sections(&list, page_size, &sections)) {
        fprintf(out, "# PID %ld 分析失败\n\n内存不足，无法构建 ELF section 索引。\n\n", (long)pid);
        vma_list_free(&list);
        return false;
    }
    if (!build_logical_regions(&list, exe_path, &exe_layout, &sections, page_size, &regions)) {
        fprintf(out, "# PID %ld 分析失败\n\n内存不足，无法构建逻辑分段。\n\n", (long)pid);
        runtime_section_list_free(&sections);
        vma_list_free(&list);
        return false;
    }
    ...
    derive_logical_pagemap_from_vmas(&regions, page_size);

    print_report_header(out, pid, comm, exe_path, pagemap_status, page_size);
    print_overview_matrix(out, &list, &regions, page_size);
    print_section_summary(out, &regions, page_size);
...
static bool analyze_one_pid_split(pid_t pid, FILE *index_out, const char *details_dir,
                                  char *detail_link, size_t detail_link_len,
                                  const ReportOptions *opts)
{
    VmaList list = {0};
    LogicalRegionList regions = {0};
    ...
    char exe_path[PATH_MAX];
    char pagemap_status[256];
    char detail_path[PATH_MAX];
    char detail_link_dir[PATH_MAX];
    ExeLayout exe_layout;
    long page_size = sysconf(_SC_PAGESIZE);
    FILE *detail = NULL;
    bool ok = false;
    ...
    read_comm(pid, comm, sizeof(comm));
    read_exe_path(pid, exe_path, sizeof(exe_path));
    sanitize_filename_component(comm[0] ? comm : "unknown", safe_comm, sizeof(safe_comm));

    if (snprintf(detail_path, sizeof(detail_path), "%s/pid_%ld_%s_details",
                 details_dir, (long)pid, safe_comm) >= (int)sizeof(detail_path)) {
        fprintf(index_out, "## PID %ld `%s`\n\n详情路径过长，无法写出。\n\n", (long)pid, comm[0] ? comm : "unknown");
        return false;
    }

    snprintf(detail_link_dir, sizeof(detail_link_dir), "details/pid_%ld_%s_details", (long)pid, safe_comm);
    snprintf(detail_link, detail_link_len, "%s/index.md", detail_link_dir);

    if (!parse_maps(pid, &list, err, sizeof(err))) {
        fprintf(index_out, "## PID %ld `%s`\n\n%s\n\n", (long)pid, comm[0] ? comm : "unknown", err);
        return false;
    }
    if (!parse_smaps_fields(pid, &list, err, sizeof(err))) {
        fprintf(index_out, "## PID %ld `%s`\n\n%s\n\n", (long)pid, comm[0] ? comm : "unknown", err);
    ...
    fprintf(index_out, "## PID %ld `%s`\n\n", (long)pid, comm[0] ? comm : "unknown");
    fprintf(index_out, "| 项目 | 值 |\n");
    fprintf(index_out, "| --- | --- |\n");
    fprintf(index_out, "| 可执行文件 | `%s` |\n", exe_path[0] ? exe_path : "(无法读取 /proc/<pid>/exe)");
    fprintf(index_out, "| 页大小 | `%ld bytes` |\n", page_size);
    fprintf(index_out, "| pagemap 状态 | %s |\n", pagemap_status);
    fprintf(index_out, "| 详细报告 | [打开](%s) |\n\n", detail_link);
    print_overview_matrix(index_out, &list, &regions, page_size);
    print_section_summary(index_out, &regions, page_size);

    if (!write_split_detail_report(detail_path, detail_link_dir, pid, comm, exe_path,
                                   pagemap_status, page_size, &list, &regions, opts)) {
        fprintf(index_out, "> 无法创建详细报告目录 `%s`: %s\n\n", detail_path, strerror(errno));
        goto done;
    }
    ok = true;
...
int main(int argc, char **argv)
{
    ...
    char run_parent[PATH_MAX];
    char details_dir[PATH_MAX];
    FILE *out = NULL;
    ReportOptions report_opts = DEFAULT_REPORT_OPTIONS;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--md-preset") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "--md-preset 后面需要 compact、standard 或 full\n");
                return 1;
            }
            if (!set_report_preset(&report_opts, argv[++i])) {
                fprintf(stderr, "未知 --md-preset: %s\n", argv[i]);
                return 1;
            }
            continue;
        }
        if (strcmp(argv[i], "--with-target-segments") == 0) {
            report_opts.target_segments = true;
            continue;
        }
        if (strcmp(argv[i], "--with-vma-details") == 0) {
            report_opts.vma_details = true;
            continue;
        }
        if (strcmp(argv[i], "--with-page-bitmaps") == 0) {
            report_opts.vma_page_bitmaps = true;
            continue;
        }
        if (strcmp(argv[i], "--with-pfn-markdown") == 0) {
            report_opts.logical_pfn_sequences = true;
            continue;
        }
        if (strcmp(argv[i], "--with-fields") == 0) {
            report_opts.fields = true;
            continue;
        }
        if (strcmp(argv[i], "--with-full-report") == 0) {
            report_opts.full_report = true;
            continue;
        }
        if (strcmp(argv[i], "--with-all-markdown") == 0) {
            report_opts = FULL_REPORT_OPTIONS;
            continue;
        }
        char *end = NULL;
        ...
        if (end == argv[i] || *end != '\0' || pid_long <= 0 || pid_long > INT_MAX) {
            ...
            if (single_app != NULL) {
                fprintf(stderr, "只能指定一个 app 关键字: 已有 `%s`，又收到 `%s`\n", single_app, argv[i]);
                return 1;
            }
            single_app = argv[i];
            continue;
    ...
    if (single_app != NULL) {
        if (pid_count != 0) {
            fprintf(stderr, "单次分析请不要混用 PID 和 app 关键字；请分别运行。\n");
            return 1;
        }
        pid_count = discover_app_pids(single_app, pids, (int)(sizeof(pids) / sizeof(pids[0])));
        if (pid_count == 0) {
            fprintf(stderr, "没有找到匹配 app 关键字 `%s` 的进程。\n", single_app);
            return 1;
        }
        fprintf(stderr, "app `%s` 匹配到 %d 个进程，将逐个分析。\n", single_app, pid_count);
    }
    ...
    if (out_path == NULL) {
        if (single_app != NULL) {
            snprintf(auto_out, sizeof(auto_out), "mem_analyze_%s.md", single_app);
        } else if (pid_count == 1) {
    ...
    if (!path_parent_dir(run_out, run_parent, sizeof(run_parent))) {
        fprintf(stderr, "无法确定报告父目录 %s: %s\n", run_out, strerror(errno));
        fclose(out);
        return 1;
    }
    if (strcmp(run_parent, ".") == 0) {
        snprintf(details_dir, sizeof(details_dir), "details");
    } else if (snprintf(details_dir, sizeof(details_dir), "%s/details", run_parent) >= (int)sizeof(details_dir)) {
        fprintf(stderr, "详细报告目录路径过长\n");
        fclose(out);
        return 1;
    }
    if (!mkdir_p(details_dir)) {
        fprintf(stderr, "无法创建详细报告目录 %s: %s\n", details_dir, strerror(errno));
        fclose(out);
        return 1;
    }

    fprintf(out, "# 进程内存映射分析报告索引\n\n");
    fprintf(out, "> 单次分析在 v3 中拆分为主索引和 `%s/` 下的详细报告；主文件保留总览、一级 segment 汇总和二级 section 汇总。\n\n",
            "details");
    ...
    for (int i = 0; i < pid_count; i++) {
        ...
        if (analyze_one_pid_split(pids[i], out, details_dir, detail_link, sizeof(detail_link), &report_opts)) {
            ok_count++;
        }
    }
