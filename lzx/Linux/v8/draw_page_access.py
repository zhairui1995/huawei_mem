#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从 page_access_collect.c 输出的 bitmap/TSV 绘制 segment 聚合图。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="根据 C 采集器输出的 young/accessed bit bitmap/TSV 绘图")
    parser.add_argument("input", type=Path, help="page_access_collect 生成的 bitmap 文本；也兼容 TSV")
    parser.add_argument("-o", "--output", type=Path, default=Path("page_access_segments.png"), help="输出 PNG")
    ...
    parser.add_argument("--title", default="young/accessed bit 操作期间访问页", help="图片标题")
